...
public class AddrSpace {
	...
	public AddrSpace(OpenFile executable) {
		...
		memoryMapping=new int[mNumPages];
		for (int i = 0; i < mNumPages; i++)
		{
			...
			{
				executable.readAt(tempStore, Machine.PageSize, noffH.code.inFileAddr + i * Machine.PageSize);
			}
			mPageTable[i].translationalMapping = JNachos.swapCounter;
			memoryMapping[i] = JNachos.swapCounter;

			JNachos.SwapSpaceFile.writeAt(tempStore, Machine.PageSize, memoryMapping[i] * Machine.PageSize);
...
////				System.arraycopy(tempStore, 0, Machine.mMainMemory, mPageTable[i].physicalPage * Machine.PageSize,
////						Machine.PageSize);
//
//				JNachos.SwapSpaceFile.writeAt(tempStore,Machine.PageSize* JNachos.swapCounter,i);
//}
