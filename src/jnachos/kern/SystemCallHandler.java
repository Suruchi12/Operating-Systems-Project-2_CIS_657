          ...
          //  Scheduler obj=new Scheduler();
            ...
            switch (pWhichSysCall) {
                ...
                case SC_Exit:
                    ...
                    System.out.println("==============\nDisk clean up\n==============");
                    for (TranslationEntry tr : MMU.mPageTable)
                    {
                            if (tr.valid)
                            {
                                //zero out all of Main Memory
                                Arrays.fill(Machine.mMainMemory, tr.physicalPage * Machine.PageSize,
                                        (tr.physicalPage + 1) * Machine.PageSize, (byte) 0);

                                AddrSpace.mFreeMap.clear(tr.physicalPage);
                                //if(tr.translationalMapping == JNachos.SchedulingFIFO.remove(tr.physicalPage))
                                    JNachos.SchedulingFIFO.clear();
                                byte bytes[] = new byte[Machine.PageSize];
                                Arrays.fill(bytes,0,Machine.PageSize,(byte)0);
                                JNachos.SwapSpaceFile.writeAt(bytes,Machine.PageSize,JNachos.swapCounter*Machine.PageSize);
